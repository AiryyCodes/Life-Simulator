...
function typeCheckPlugin(tsConfigPath, sourcePath) {
	return {
		name: "typecheck",
		buildStart() {
			...
			// Run tsc with proper stdio piping
			try {
				console.log(`Running typecheck...`);

				const normalizedPath = tsConfigPath.replace(/\\/g, "/");

				execSync(`tsc --noEmit --project "${normalizedPath}"`, { stdio: "inherit" });
...
const generateConfig = (options = {}) => {
	...
	return {
		...
		plugins: [
			// typeCheckPlugin(resolvePath([sourcePath, isServer ? "server" : "client", "tsconfig.json"]), sourcePath),
